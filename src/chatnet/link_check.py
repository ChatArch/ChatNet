...
def _request(url: str, timeout: float, method: str) -> int:
    req = Request(url, method=method, headers={"User-Agent": "chatnet-link-check/1.0", "Accept": "*/*"})
    ...
    try:
        ...
        if exc.code in {400, 403, 405}:
            try:
                ...
                elapsed_ms = int((time.monotonic() - start) * 1000)
                return LinkCheckResult(url=url, ok=200 <= status < 400, status=status, elapsed_ms=elapsed_ms, error=None)
            except Exception as retry_exc:  # noqa: BLE001 - preserve user-facing error context
                elapsed_ms = int((time.monotonic() - start) * 1000)
                return LinkCheckResult(url=url, ok=False, status=None, elapsed_ms=elapsed_ms, error=str(retry_exc))
        elapsed_ms = int((time.monotonic() - start) * 1000)
...
def _request_text(url: str, timeout: float, max_bytes: int) -> tuple[int, str]:
    req = Request(url, method="GET", headers={"User-Agent": "chatnet-link-check/1.0", "Accept": "*/*"})
    context = ssl.create_default_context()
    with urlopen(req, timeout=timeout, context=context) as resp:
...
def check_service_url(url: str, expected: Optional[str], timeout: float, max_bytes: int = 65536) -> ServiceCheckResult:
    ...
    try:
        ...
        if expected_lower:
            ...
            matched = expected_lower in content_lower
        ...
        return ServiceCheckResult(url=url, expected=expected, ok=ok, status=status, elapsed_ms=elapsed_ms, matched=matched, error=error)
    except HTTPError as exc:
        elapsed_ms = int((time.monotonic() - start) * 1000)
        return ServiceCheckResult(url=url, expected=expected, ok=False, status=exc.code, elapsed_ms=elapsed_ms, matched=False, error=str(exc))
    except (URLError, OSError, TimeoutError) as exc:
        elapsed_ms = int((time.monotonic() - start) * 1000)
        return ServiceCheckResult(url=url, expected=expected, ok=False, status=None, elapsed_ms=elapsed_ms, matched=False, error=str(exc))
